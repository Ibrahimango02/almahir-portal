...
export default function ParentReportsPage() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                setLoading(true)
                const sessionsData = await getParentSessionHistoryForReports(currentParentId)
                setSessions(sessionsData)
            } catch (err) {
                console.error('Error fetching parent session history:', err)
                                ...
                                <tbody className="bg-white divide-y divide-gray-100">
                                    {filteredSessions.map((session) => (
                                        <tr
                                            ...
                                            onClick={() => router.push(`/parent/classes/${session.class_id}/${session.session_id}`)}
